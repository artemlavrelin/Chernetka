...
def promotion_menu_keyboard(balance: int = 0) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📂 Pull — лента заданий",  callback_data="pull_start")],
        [InlineKeyboardButton(text="☑️ Создать задание",        callback_data="create_task")],
        [InlineKeyboardButton(text="🪪 Верификация",            callback_data="verification_menu")],
        [InlineKeyboardButton(text="◀️ Назад",                  callback_data="main_menu")],
    ])


def task_type_keyboard(balance: int = 0) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
